 ...
 */
	...
	$.jstree.core.prototype = {
			...
			this.element
						...
						switch(e.which) {
							...
							case 37: // right
								...
								if(this.is_open(e.currentTarget)) {
									...
									if(o && o.id !== $.jstree.root) { this.get_node(o).children('.jstree-anchor').focus(); }
								}
								break;
							case 38: // up
								e.preventDefault();
							...
							case 39: // left
								...
								if(this.is_closed(e.currentTarget)) {
									this.open_node(e.currentTarget, function (o) { this.get_node(o).children('.jstree-anchor').focus(); });
								}
								else if (this.is_open(e.currentTarget)) {
									o = this.get_node(e.currentTarget).children('.jstree-children')[0];
				...
				.on('focus.jstree', $.proxy(function () {
						if(+(new Date()) - was_click > 500 && !this._data.core.focused) {
							was_click = 0;
							var act = this.get_node(this.element.attr('aria-activedescendant'));
							if(act) {
		...
		load_node : function (obj, callback) {
			...
			this.get_node(obj).addClass("jstree-loading").attr('aria-busy',true);
			this._load_node(obj, $.proxy(function (status) {
				obj = this._model.data[obj.id];
				obj.state.loading = false;
				obj.state.loaded = status;
				obj.state.failed = !obj.state.loaded;
				var dom = this.get_node(obj), i = 0, j = 0, m = this._model.data, has_children = false;
		...
		open_all : function (obj, animation, original_obj) {
			...
			obj = this.get_node(obj);
			if(!obj) { return false; }
			var dom = obj.id === $.jstree.root ? this.get_container_ul() : this.get_node(obj), i, j, _this;
			if(!dom.length) {
		...
		close_all : function (obj, animation) {
			...
			obj = this.get_node(obj);
			if(!obj) { return false; }
			var dom = obj.id === $.jstree.root ? this.get_container_ul() : this.get_node(obj),
				_this = this, i, j;
			if(dom.length) {
		...
		enable_node : function (obj) {
			...
			this.get_node(obj).children('.jstree-anchor').removeClass('jstree-disabled').attr('aria-disabled', false);
			/**
		...
		disable_node : function (obj) {
			...
			this.get_node(obj).children('.jstree-anchor').addClass('jstree-disabled').attr('aria-disabled', true);
			/**
		...
		refresh_node : function (obj) {
			...
			this.get_node(obj).find('.jstree-open').each(function() { opened.push(this.id); });
		...
		delete_node : function (obj) {
			...
			if($.inArray(this._data.core.focused, tmp) !== -1) {
				...
				if(par.id === $.jstree.root) {
					this.get_node(this._model.data[$.jstree.root].children[0]).children('.jstree-anchor').focus();
				}
				else {
					this.get_node(par).children('.jstree-anchor').focus();
				}
		...
		set_icon : function (obj, icon) {
			...
			dom = this.get_node(obj).children(".jstree-anchor").children(".jstree-themeicon");
			if(icon === false) {
				this.hide_icon(obj);
			}
		...
		hide_icon : function (obj) {
			...
			obj.icon = false;
			this.get_node(obj).children(".jstree-anchor").children(".jstree-themeicon").addClass('jstree-themeicon-hidden');
			return true;
		},
		/**
					...
					.on(this.settings.checkbox.tie_selection ? 'deselect_node.jstree' : 'uncheck_node.jstree', $.proxy(function (e, data) {
							...
							if(s.indexOf('up') !== -1) {
								for(i = 0, j = obj.parents.length; i < j; i++) {
									...
									tmp = this.get_node(obj.parents[i]);
									if(tmp && tmp.length) {
		...
		this.bind = function () {
			...
			this.element
				...
				.on("changed.jstree", $.proxy(function (e, data) {
						...
						var tmp = false, i, j;
						for(i = 0, j = data.selected.length; i < j; i++) {
							tmp = this.get_node(data.selected[i]);
							if(tmp && tmp.length) {
				...
				.on("open_node.jstree", $.proxy(function (e, data) {
						this.get_node(data.node).find('.jstree-clicked').parent().children('.jstree-wholerow').addClass('jstree-wholerow-clicked');
					}, this))
				.on("hover_node.jstree dehover_node.jstree", $.proxy(function (e, data) {
						if(e.type === "hover_node" && this.is_disabled(data.node)) { return; }
						this.get_node(data.node).children('.jstree-wholerow')[e.type === "hover_node"?"addClass":"removeClass"]('jstree-wholerow-hovered');
					}, this))
